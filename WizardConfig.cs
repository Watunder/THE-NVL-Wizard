...
namespace Wizard
{
    ...
    class WizardConfig
    {
        ...
        public static readonly string DEFAULT_THEME_NAME = Localize("默认主题");
        ...
        public bool IsReady(TextWriter output)
        {
            try
            {
                ...
                {
                    if (output != null) output.WriteLine(Localize("错误：软件根目录不存在。"));
                    return false;
                }

                if (_height <= 0 || _width <= 0)
                {
                    if (output != null) output.WriteLine(Localize("错误：无效的分辨率设置。"));
                    return false;
                }
                ...
                {
                    if (output != null) output.WriteLine(Localize("错误：无效的项目名称。"));
                    return false;
                }
                else if (!this.IsModifyProject && Directory.Exists(path))
                {
                    if (output != null) output.WriteLine(Localize("错误：项目文件夹已存在，请更换项目名或设置其他路径。"));
                    return false;
                }
                ...
                if (this.IsModifyProject)
                {
                    ...
                    {
                        if (output != null) output.WriteLine(Localize("错误：项目分辨率未改动，不需要进行转换"));
                        return false;
                    }
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        {
                            if (output != null) output.WriteLine(Localize("错误：主题目录不存在。"));
                            return false;
                        }
                        ...
                        {
                            if (output != null) output.WriteLine(Localize("警告：主题缺少配置文件"));
                        }
                    }
            ...
            {
                if (output != null) output.WriteLine(Localize("无效的项目配置：") + e.Message);
                return false;
            }
        ...
        public override string ToString()
        {
            ...
            if (this.IsModifyProject)
            {
                sb.AppendFormat(Localize("【修改项目配置清单】")); sb.Append(Environment.NewLine);

                sb.Append(Environment.NewLine);
                ProjectProperty info = this.ProjectInfo;
                sb.AppendFormat(Localize("原始分辨率：{0}x{1}"), info.width, info.height);sb.Append(Environment.NewLine);
                sb.AppendFormat(Localize("目标分辨率：{0}x{1}"), this._width, this._height);
                if (info.width != _width || info.height != this._height)
                {
                    sb.AppendFormat(Localize(" (分辨率已修改)"));
                }
                sb.Append(Environment.NewLine);
            }
            else
            {
                sb.AppendFormat(Localize("【创建项目配置清单】")); sb.Append(Environment.NewLine);

                sb.Append(Environment.NewLine);
                string theme = (this.IsDefaultTheme) ? DEFAULT_THEME_NAME : this.ThemeName;
                sb.AppendFormat(Localize("所选主题：{0}"), theme); sb.Append(Environment.NewLine);
                sb.AppendFormat(Localize("分辨率设定：{0}x{1}"), this._width, this._height);
                ProjectProperty info = this.ThemeInfo;
                if (info.width != _width || info.height != this._height)
                {
                    sb.AppendFormat(Localize(" (非原始分辨率)"));
                }
                sb.Append(Environment.NewLine);
            }

            sb.Append(Environment.NewLine);
            sb.AppendFormat(Localize("项目名称：{0}"), this._projectName);sb.Append(Environment.NewLine);
            sb.AppendFormat(Localize("项目位置：{0}"), this.ProjectFolder); sb.Append(Environment.NewLine);
            
            sb.Append(Environment.NewLine);
            sb.AppendFormat(Localize("缩放策略：{0}"), this._scaler); sb.Append(Environment.NewLine);
            sb.AppendFormat(Localize("缩放质量：{0}"), this._quality); sb.Append(Environment.NewLine);
            sb.AppendFormat(Localize("NVLMaker目录：{0}"), this.BaseFolder);sb.Append(Environment.NewLine);
            return sb.ToString();
        }
    ...
    class WizardConverter : ResConverter
    {
        ...
        void ConvertFiles(string srcPath, int sw, int sh, string destPath, int dw, int dh)
        {
            ...
            try
            {
                ...
                foreach (string srcfile in srcFiles)
                {
                    ...
                    {
                        // 直接拷贝
                        OnLogging(string.Format(Localize("拷贝{0}"), relFile));
                        File.Copy(srcfile, Path.Combine(destPath, relFile), true);
                    }
                }

                OnLogging(Localize("图片转换中……"));
        ...
        void AdjustSettings(int sw, int sh)
        {
            ...
            {
                OnError(Localize("修改setting.tjs失败:") + e.Message);
            }
            ...
            {
                OnError(Localize("修改Config.tjs失败:") + e.Message);
            }
            ...
            if (sw != dw || sh != dh)
            {
                ...
                {
                    OnError(Localize("修改界面布局文件失败:") + e.Message);
                }
            }
        }
